...
const connectionString =
  "mongodb+srv://Vulcan12:Notstarwars12@cluster0.uw2owll.mongodb.net/?retryWrites=true&w=majority";
...
MongoClient.connect(connectionString, {
  ...
  .then((client) => {
    ...
    app.get("/api/:alienName", (request, response) => {
      const aliensName = request.params.alienName.toLowerCase(); // pulls name that's typed
      infoCollection
        .find({ name: aliensName })
        .toArray() // turned to array because it does not find data unless it's in an array
        .then((results) => {
          console.log(results);
          response.json(results[0]); // added [0] so that it reaches into the object document
        })
